...
/*
 ...
 $m = strval(intval($d[1])+1);
 $expDate = $d[0] . '-' . $m . '-' . $d[2];
 echo $expDate;
*/

//inserting into users table
$query="insert into users(username,gender,mobile,email,dob,userid,status,j_date) values('$uname','$gender','$phn','$email','$dob','$memID',1,'$jdate')";
    if(mysqli_query($con,$query)==1){
      ...
      $result=mysqli_query($con,$query1);
        ...
        if($result){
          ...
          $query2="insert into enrolls_to(pid,uid,join_date,expire,renewal) values('$plan','$memID','$cdate','$expiredate','yes')";
